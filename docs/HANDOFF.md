...
| | |
|---|---|
| Tests | 353 passed, 4 skipped (`ruff` and `ruff format` clean) |
| Last done | `TASK-305` — Readable playlist tree names |
...
| 44 | ~~`TASK-305`~~ | Playlist tree names use the live pane width |
| 45 | `TASK-303` | Analysis-aware track colour and honest `x/y` |
...
| `adapters/serato/tags.py` | reads/writes GEOB (v2.3/v2.4) and GEO (v2.2) in MP3 / WAV / AIFF, Vorbis comments on FLAC, and `----:com.serato.dj` atoms on M4A / MP4; `read_geob` and skip read only the tag; verifies audio hash and frame read-back; same-size writes patch the dirty span; tagless WAV appends `id3 `; other size-changing writes fsync a sibling `.tmp` and restore the original if replace leaves a short file |
...
TASK-305. Next is `TASK-303`: yellow when a track is in the crate but
