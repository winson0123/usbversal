...
| ~~`TASK-204`~~ | ~~Progress rate + ETA~~ | Done — `ProgressRateTracker` (windowed rate as of TASK-276). |
...
| ~~`TASK-317`~~ | ~~Library paints before analysis colours finish~~ | Done — Home leaves as soon as the USB opens; crate membership fills the tree first; analysis colours follow. Track preview no longer cancels that refresh. |
