...
| ~~`TASK-075`~~ | ~~Nested playlist folders → `Parent%%Child.crate`~~ | Done — `crate_name_for()` walks ancestor folders, joined with `%%`. Confirmed in Serato (TASK-245): `Gigs → Safety Day` shows as a parent-child crate. No empty parent crate file required. |
