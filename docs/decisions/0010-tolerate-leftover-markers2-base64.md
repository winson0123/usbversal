...
`COLOR` and `BPMLOCK` survive while cues come from Rekordbox. Python
3.11+ `base64.b64decode` rejects a data length of `4n+1`.

A Contents-crate scan found 23 MP3s with that leftover. The 69-character
body that matched the error (Thank u, next):
...
No bytes left. One extra base64 character cannot encode an extra byte.
That needs two characters. Dropping it removes no field.
