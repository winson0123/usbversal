...
### Nested playlist folders [assumed]
...
Not exercised in the experiment (the test playlist was top-level). **usbversal
does not currently do this** — it flattens to the leaf name, so two playlists
sharing a name in different folders collide.
