...
WONSIN M4As after TASK-263/285 carried `beatgrid`, `markersv2`, and
`markers`. The grid showed in Serato, slightly late. Pads 1-5 did not.
...
`otonoke` `iTunSMPB` delay is `0x840` (2112 samples) at 44100 Hz ≈ 48 ms.
The first ANLZ beat is 47 ms. Files without `iTunSMPB` use 2112 samples
and the `mdhd` timescale (44 ms at 48000 Hz).

Sync subtracts that delay from beats and cues before encoding. Times
are clamped at 0.

The identified pad: `Rock That Body (Lumarii Remix)` — one hot cue on
the first beat (ANLZ 70 ms).
