...
`write_geob` rebuilds the file in memory and checks the audio hash plus
...
frames, AIFF `SSND`, FLAC audio, and MP4 `mdat` are seeked past so a
50 MB song is not pulled just to see that BeatGrid already matches.
A rewrite still loads the whole file.

When the rebuilt file is the same length (padded MP3, or a WAV that
already has `id3 `), only the changed span is written in place and
fsynced. No sibling `.tmp`, no `replace`. A failed patch writes that
span back from the in-memory original.
...
That commit patches those four bytes and appends the tail. A failed
append restores the original size and RIFF size.

A WAVE whose `id3 ` already sits after `data` may grow that chunk.
Rekordbox often writes a tight tag with a `LIST` after it. Enlarging
...
gets an ID3v2.4 tag prepended, so the audio moves and that path still
replaces. A RIFF file that is not WAVE is still rejected.
...
- If the destination is missing or short after the swap, write the
  original bytes back.
- If the destination is empty and a leftover `.tmp` has bytes, promote
...
The same swap without fsync left every `_Serato_` file at 0 bytes after
a fresh export was synced and the stick disconnected at 03:35
(`lost async page write`. Remount: `Volume was not properly unmounted`).
Windows then offered scan-and-fix. Contents was intact.
...
flushes the volume (`syncfs`). Unmount yourself before Windows or
Serato. The TUI does not eject. See
`docs/workflows/volume-flush.md`.

Reading a leftover 0-byte `.crate` used to crash the Library screen
(`Crate()`: `version not set after parsing file`).
`read_crate_track_paths` returns no tracks for an empty or unparseable
file.
